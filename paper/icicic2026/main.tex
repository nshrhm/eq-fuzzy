...
The planned ICICIC 2026 experiment combines a benchmark coverage matrix with a
...
registry, four emotions, a budget model panel, and repeated sampling.
...
\section{Planned Results and Artifact Chain}
The primary outputs are generated artifacts rather than manually edited result
tables.
The coverage table is written to the ICICIC comparison-matrix artifact
directory.
After raw matched-subset responses are collected, the analysis script writes
cell summaries, model summaries, target-shift summaries, and response-level
profile entropy to the ICICIC matched-subset analysis artifact directory.
Manuscript-facing tables and figures are then regenerated by
the primary table and figure build scripts.

External mini-comparison items are intentionally gated on manual curation.
Only public items with clear source URLs and reuse notes should be entered in
the external benchmark curation file.
SECEU is kept as a coverage-matrix comparator unless item reuse terms are
explicitly clear.
